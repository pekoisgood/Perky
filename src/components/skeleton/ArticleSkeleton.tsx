...
const contentClass = `w-full h-[20px] rounded-full ${skeletonAnimation}`;

const ArticleSkeleton = () => {
  return (
    <div className="w-full max-w-[800px] rounded-lg flex flex-col gap-2 items-center justify-center mx-auto py-10">
      <div className="relative w-full">
        <h1 className={`w-full h-[40px] rounded-full ${skeletonAnimation}`} />
      </div>
      <p
        className={`${skeletonAnimation} w-full max-w-[300px] h-[30px] rounded-full`}
      />
      <p
        className={`${skeletonAnimation} w-full max-w-[100px] h-[30px] rounded-full`}
      />
